#!/usr/bin/env python3
"""
VOICE LEARNED - Speaks using learned knowledge from base.txt
No external APIs. Pure local learning and response.
"""
...
class VoiceLearned:
    def __init__(self, root):
        self.root = root
        self.root.title("Voice Claw - Learned System")
    ...
    def find_response(self, user_input):
        """Smart response using learned knowledge"""
        user_lower = user_input.lower()
        
        # Check conversation database first
        ...
        for pattern in self.patterns:
            ...
            if any(word in pattern_words for word in user_lower.split()):
                return f"Regarding {pattern['topic']}: {pattern['summary']}"
        ...
        if vocab_matches:
            matches_str = ', '.join(list(vocab_matches)[:3])
            return f"I understand you're asking about: {matches_str}. I've learned these topics from my knowledge base."
        
        # Generic response with learned flavor
        return f"You said: '{user_input}'. I'm still learning. Try asking about intelligence, voice, systems, or data."
    ...
    def setup_ui(self):
        ...
        title = tk.Label(
            header, text="VOICE CLAW - Self-Learned System",
        ...
        info = tk.Label(
            self.root, 
            text=f"Loaded: {len(self.conversations)} conversations | {len(self.concepts)} concepts | {len(self.vocab)} words",
        ...
        response_label = tk.Label(
            content, text="LEARNED RESPONSE:", font=("Arial", 11, "bold"),
            bg="#1a1a1a", fg="#00ff00"
        )
...
"""
            ...
            if text and "[No speech" not in text:
                ...
                self.voice_text.delete(1.0, tk.END)
                self.voice_text.insert(1.0, "[No speech]")
