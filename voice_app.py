#!/usr/bin/env python3
"""
VOICE APP - Completely Local (Windows Speech API)
No external servers needed. Uses Windows built-in speech recognition + TTS
"""
...
class VoiceApp:
    ...
    def setup_ui(self):
        ...
        self.speak_btn = tk.Button(
            ...
            width=20, height=2, cursor="hand2", state=tk.DISABLED
        )
    ...
    def record_and_process(self):
        try:
            ...
            self.status_var.set("Recording (6 seconds)...")
            ...
            try:
                ...
                self.voice_text.delete(1.0, tk.END)
                self.voice_text.insert(1.0, "[Could not understand audio]")
            ...
            self.status_var.set("Processing with OpenClaw...")
            ...
            if result.returncode == 0:
                ...
                if response:
                    ...
                    self.response_text.delete(1.0, tk.END)
                    self.response_text.insert(1.0, "No response from OpenClaw")
                    self.status_var.set("Error: no response")
            else:
                self.response_text.delete(1.0, tk.END)
                self.response_text.insert(1.0, f"Error: {result.stderr}")
                self.status_var.set("Error processing")
