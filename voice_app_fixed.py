#!/usr/bin/env python3
"""
VOICE APP FIXED - Explicit device selection
"""
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if response:
                        ...
                        self.response_text.delete(1.0, tk.END)
                        self.response_text.insert(1.0, "No response")
                        self.status_var.set("No response from OpenClaw")
                else:
                    self.response_text.delete(1.0, tk.END)
                    self.response_text.insert(1.0, f"Error: {result.stderr[:200]}")
                    self.status_var.set("Processing error")
                    
            except sr.UnknownValueError:
                self.voice_text.delete(1.0, tk.END)
                self.voice_text.insert(1.0, "[Could not understand audio - please speak louder]")
                self.status_var.set("Could not understand. Try again.")
            except sr.RequestError as e:
                self.voice_text.delete(1.0, tk.END)
                self.voice_text.insert(1.0, f"[Speech API error: {str(e)}]")
